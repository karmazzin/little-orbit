...
export type ContentState={postcards:'new'|'active'|'complete';cards:string[];sketches?:string[];iris:'dormant'|'searching'|'observed'|'complete';clues:string[]};
export type ContentAction={type:'accept'|'deliver'|'observe'|'decode'}|{type:'card'|'clue'|'sketch';id:string};
export const initialContent=():ContentState=>({postcards:'new',cards:[],sketches:[],iris:'dormant',clues:[]});
export function reduceContent(s:ContentState,a:ContentAction):ContentState{
 ...
 if(a.type==='sketch'&&s.postcards==='complete'&&sketchPlace(a.id)&&!(s.sketches??[]).includes(a.id))return {...s,sketches:[...(s.sketches??[]),a.id]};
...
export function restoreContent(raw:string|null):ContentState{
 ...
 if(s.postcards==='complete'&&Array.isArray(v.sketches))s.sketches=[...new Set<string>(v.sketches.filter((id:unknown)=>typeof id==='string'&&sketchPlace(id)))];
...
export const CLUE_TEXT:Record<string,string>={
 ...
 grove:'На обратной стороне таблички — тонкая латунная пластинка. Три короткие черты и одна длинная окружены схемой кольца. Ниже подпись: «Смотри с уступа, когда Ореол скроется». Ты переносишь рисунок в журнал.',
};
