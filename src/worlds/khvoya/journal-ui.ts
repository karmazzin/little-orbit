import {journeyEntries,JOURNEY_STATUS,type JourneyEntry,type JourneySnapshot} from '../../journey.ts';
import {journalFindings,journalWorld,journalStories,journalEnding} from './journal.ts';
...
export function createJournalUI(host:{track:(id:string)=>void}){
 ...
 function render(){
  const unstarted=journeyEntries(snapshot,'unstarted');
  const world=journalWorld(snapshot),findings=journalFindings(snapshot,campFound),active=journalStories(rows,'active').filter(row=>!unstarted.some(start=>start.id===row.id)),complete=journalStories(rows,'complete');
  const counts:Record<string,number>={world:world.length,unstarted:unstarted.length,active:active.length,findings:findings.length,complete:complete.length};
  const labels:Record<string,string>={world:'Мир',unstarted:'Не начато',active:'В процессе',findings:'Находки',complete:'Завершённые'};
  ...
  if(!counts[section]){body.append(text(section==='world'?'Здесь появятся твои нарисованные открытки. После передачи альбома Мире можно рисовать любое место на карте: подойди к нему и нажми P.':section==='unstarted'?'Все истории уже начаты или завершены.':section==='active'?'Пока нет начатых историй. Поговори с Мирой у почтового домика или отправляйся исследовать долину.':section==='findings'?'Здесь появятся открытые места и найденные записи.': 'Здесь будут храниться завершённые истории.'));return;}
  if(section==='unstarted'){
   body.append(text('Как начать миссии, за которые ты ещё не брался.'));
   for(const row of unstarted){const article=document.createElement('article');article.className='journey-row';const title=document.createElement('h3');title.textContent=row.title;article.append(title,text('Как начать: '+row.next));body.append(article);}
   return;
  }
  if(section==='world'){const gallery=document.createElement('div');gallery.className='journal-postcards';gallery.innerHTML=world.map(f=>postcardMarkup(f.card!)).join('');body.append(gallery);return;}
