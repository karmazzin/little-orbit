...
function obstructed(up:Vector3,obstacles:Obstacle[]){
...
function shoreTarget(p:Player,obstacles:Obstacle[]):Vector3|null{
 ...
 for(let distance=.25;distance<=2.5;distance+=.25){
  for(let i=0;i<16;i++){
   ...
   for(let j=1;j<=steps;j++){
    ...
    if(surface.bridge||obstructed(at,obstacles)||Math.abs(b-a)>stride*1.05+.015){valid=false;break;}
    previous=surface;
   }
...
export function movementAction(p:Player,obstacles:Obstacle[]):MovementMode|null{
 return shoreTarget(p,obstacles)?p.mode==='walk'?'swim':'walk':null;
}
...
export function step(p:Player,input:Input,dt:number,obstacles:Obstacle[]){
 ...
 p.moving=false;
 if(input.toggleMode){
  const target=shoreTarget(p,obstacles);
 ...
 if(distance>1e-8){
  ...
  if(!waterBlocked&&!slopeBlocked&&!obstructed(next,obstacles)){
